...
public class BlockMonolithIcebolt extends BlockMonolithBase
{
    public BlockMonolithIcebolt(Material material)
    {
		super(material);
	}
    ...
    {
        this.iconFront = iconRegister.registerIcon(RunicInscription.ID + ":" + "Monolith2");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemScrollIcebolt, 1));
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemPaperTracedIcebolt, 1));
    		if(!player.capabilities.isCreativeMode)
    		{
    			player.inventory.consumeInventoryItem(heldItem.getItem());
    		}
