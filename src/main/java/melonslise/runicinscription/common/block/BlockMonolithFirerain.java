...
public class BlockMonolithFirerain extends BlockMonolithBase
{
	public BlockMonolithFirerain(Material material)
    {
		super(material);
	}
    ...
    {
        this.iconFront = iconRegister.registerIcon(RunicInscription.ID + ":" + "Monolith6");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemScrollFirerain, 1));
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemPaperTracedFirerain, 1));
    		if(!player.capabilities.isCreativeMode)
    		{
    			player.inventory.consumeInventoryItem(heldItem.getItem());
    		}
