...
public class BlockMonolithFirebolt extends BlockMonolithBase
{
	public BlockMonolithFirebolt(Material material)
	{
		super(material);
	}
    ...
    {
        this.iconFront = iconRegister.registerIcon(RunicInscription.ID + ":" + "Monolith1");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemScrollFirebolt, 1));
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemPaperTracedFirebolt, 1));
    		if(!player.capabilities.isCreativeMode)
    		{
    			player.inventory.consumeInventoryItem(heldItem.getItem());
    		}
