...
public class BlockMonolithDispel extends BlockMonolithBase
{
	public BlockMonolithDispel(Material material)
    {
		super(material);
	}
    ...
    {
        this.iconFront = iconRegister.registerIcon(RunicInscription.ID + ":" + "Monolith5");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemScrollDispel, 1));
    	...
    	{
    		player.inventory.addItemStackToInventory(new ItemStack(RunicInscription.itemPaperTracedDispel, 1));
    		if(!player.capabilities.isCreativeMode)
    		{
    			player.inventory.consumeInventoryItem(heldItem.getItem());
    		}
